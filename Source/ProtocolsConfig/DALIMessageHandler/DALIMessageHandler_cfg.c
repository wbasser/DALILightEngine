...
void DALIMessageHandler_OutputXmtMsg( U8 nResponse )
{
    ...
    DbgAscCommandHandler_OutputDaliXmt( nResponse );
  #endif
}
 ...
 * @param[in]   bBypassLockByte bypass the lock byte
 *
...
BOOL DALIMessageHandler_WriteMemoryBlock( U8 nMemoryBlock, U8 nBlockAddress, U8 nValue, BOOL bBypassLockByte )
{
  ...
  {
    case 0 :
      eParamEnum = DALI_PARAM_MB0_BASE_ENUM;
      break;

    case 1 :
      eParamEnum = DALI_PARAM_MB1_BASE_ENUM;
      break;

    default :
      eParamEnum = PARAMSEL_ILLEGAL;
      break;
  }

  // now check for a legal memory block
  if ( eParamEnum != PARAMSEL_ILLEGAL )
  {
    // add the block address
    eParamEnum += nBlockAddress;

    // write the balue
    bStatus = DALIMessageHandler_PutValue( eParamEnum, nValue, bBypassLockByte );
  }
...
BOOL DALIMessageHandler_ReadMemoryBlock( U8 nMemoryBlock, U8 nBlockAddress, PU8 pnValue )
{
  ...
  if ( eParamEnum != PARAMSEL_ILLEGAL )
  {
    ...
    // write the balue
    bStatus = DALIMessageHandler_GetValue( eParamEnum, pnValue );
  }
...
BOOL DALIMessageHandler_GetDefaultValue( U8 nParam, PU8 pnValue )
{
  ...
  // call the parameter write routine
  bStatus = ( PARAM_ERR_NONE == ParameterManager_GetDefaultValue(( PARAMSELENUM )nParam, ( PPARAMARG )pnValue )) ? TRUE : FALSE; 
 ...
 * @param[in[     bBypassLock bypass the lock
 *
 * @return    TRUE if OK, FALSE if not
 *
 *****************************************************************************/
BOOL DALIMessageHandler_PutValue( U8 nParam, U8 nValue, BOOL bBypassLock )
{
  BOOL bStatus = FALSE;

  // call the parameter write routine
  bStatus = ( PARAM_ERR_NONE == ParameterManager_PutValue(( PARAMSELENUM )nParam, ( PARAMARG )nValue, bBypassLock )) ? TRUE : FALSE; 
...
BOOL DALIMessageHandler_GetValue( U8 nParam, PU8 pnValue )
{
  ...
  // call the parameter write routine
  bStatus = ( PARAM_ERR_NONE == ParameterManager_GetValue(( PARAMSELENUM )nParam, ( PPARAMARG )pnValue )) ? TRUE : FALSE; 
...
{
  PARAMSELENUM  eParamSel;
  PARAMARG      xParamArg;

  // for each parameter in the list
  for ( eParamSel = DALI_PARAM_RST_FIRST_ENUM; eParamSel <= DALI_PARAM_RST_LAST_ENUM; eParamSel++ )
  {
    // read the value
    ParameterManager_GetDefaultValue( eParamSel, &xParamArg );

    // now write it
    ParameterManager_PutValue( eParamSel, xParamArg, TRUE );
  }
}
